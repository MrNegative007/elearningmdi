    ...
    public function listUjian(Request $request, $id){
        ...
        if($request->ajax()){
            return DataTables::of($exam)
                ...
                ->addColumn('soal_pilgan', function($row)use($id,$user,$question){
                    if(is_null($row->soal_1)){
                        ...
                        if($question->user_id != $user->id && !$user->hasRole('admin')){
                ...
                ->addColumn('soal_essay', function($row)use($id,$user,$question){
